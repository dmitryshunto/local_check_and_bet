...
const TOGGLE_IS_GETTING_DATA = 'createNewUser_REDUCER/TOGGLE_IS_GETTING_DATA'
const SET_CREATING_OF_NEW_USER = 'createNewUser_REDUCER/SET_CREATING_OF_NEW_USER'
const SET_INITIAL_STATE = 'createNewUser_REDUCER/SET_INITIAL_STATE'
const SET_defaultPhotoURL = 'createNewUser_REDUCER/SET_defaultPhotoURL'
const SET_PHOTO_FILE = 'createNewUser_REDUCER/SET_PHOTO_FILE'
const TOGGLE_IS_LOADING_PHOTO = 'createNewUser_REDUCER/TOGGLE_IS_LOADING_PHOTO'
...
export const actions = {
    ...
    set_defaultPhotoURL: (defaultPhotoURL: string) => ({ type: SET_defaultPhotoURL, defaultPhotoURL } as const),
...
let createNUReducer = (state = innitialObject, action: ActionsTypes): typeof innitialObject => {
    if (action.type === TOGGLE_IS_GETTING_DATA) {
        ...
        return {...innitialObject,
            defaultPhotoURL: state.defaultPhotoURL
        }
    } else if (action.type === SET_defaultPhotoURL) {
        return {
            ...state,
            defaultPhotoURL: action.defaultPhotoURL
        }
...
export const set_info_for_creating_userTC = (): BaseThunkActionType<ActionsTypes> => async (dispatch) => {
    ...
    const response = await usersAPI.getCreateNewUserPageInfo()
    if(response.resultCode === ResultCodeTypes.Success) {
        dispatch(actions.set_defaultPhotoURL(response.data.defaultPhotoURL))
...
export const createNewUserTC = (login: string, password: string): BaseThunkActionType<ActionsTypes> => async (dispatch, getState) => {
    ...
    let response = await usersAPI.createNewUser(login, password);
